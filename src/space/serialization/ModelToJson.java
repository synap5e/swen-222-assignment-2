...
public class ModelToJson implements WorldSaver{
	...
	private MyJsonObject constructRoom(Integer roomId, Room room) {
		...
		r.put("walls with doors", constructWalls(room)); 
		return r;
	}
	...
	private MyJsonList constructWalls(Room room) {
		...
		wall.put(entry.getKey().toString(), constructDoors(entry.getValue()));
		walls.add(wall);
		}
		return walls;
	}
	...
	private MyJsonList constructDoors(List<Door> doorsList) {
		MyJsonList doors = new MyJsonList();
		for(Door d:doorsList){
			doors.add(constructEntity(d));
		}
		return doors;
	}
